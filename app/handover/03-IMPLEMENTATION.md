...
> router, a chain activated with no factory, a missing site URL.
...
The site URL (env `VITE_SITE_URL`, or the committed `src/site.config.json` `siteUrl`) is
replaced into `index.html` at build time. Unset → empty string → relative OG URLs.
(`sitemap.xml` **is regenerated by every build** from that same origin — stub when none is
set; `robots.txt` is not auto-rewritten — fill its `Sitemap:` line manually; see §6.4.)
...
### 6.4 `sitemap.xml` is build-generated; fill `robots.txt` in by hand

- `public/sitemap.xml` — **regenerated by every build** (the `prebuild` hook) from your
  site URL (`src/site.config.json` `siteUrl`, or `VITE_SITE_URL`) and the enabled public
  routes. No origin set → the build writes an origin-less stub. Never hand-maintain it.
- `public/robots.txt` — ships with a commented `Sitemap:` placeholder; add your absolute
  `Sitemap:` URL yourself (not rewritten by the build).
...
| **OG/social card shows no image or wrong origin** | Site URL unset → relative OG URLs and a stub sitemap. | Set your site URL (setup studio / `src/site.config.json`, or `VITE_SITE_URL`) and **rebuild** — the build brands the tags and regenerates the sitemap; fill `robots.txt`'s `Sitemap:` line yourself (§6.4). |
