...
├─ public/                    favicon, PWA icons, og.png, manifest, robots.txt, sitemap.xml (build-generated),
...
**OG / social-tag substitution:** `index.html` carries static OG + Twitter `summary_large_image` tags whose four URLs use the `%VITE_SITE_URL%` placeholder (`og:url` = `%VITE_SITE_URL%/`, `og:image`/`twitter:image` = `%VITE_SITE_URL%/og.png`). The mechanism is the `siteHtml()` plugin in `vite.config.ts`, which resolves the origin from a real `VITE_SITE_URL` env var, `.env.local`, or the committed `src/site.config.json` `siteUrl` (the setup studio's home for it), in that order. **No origin anywhere → the token becomes an empty string**, yielding relative `og:url="/"` and `og:image="/og.png"`. (`public/robots.txt` has a commented `Sitemap:` placeholder to fill in manually; `public/sitemap.xml` is **regenerated by every build** from the same resolved origin — an origin-less build writes a stub.)
...
1. **Static-site SEO / OG prerender limit.** A client-rendered SPA cannot emit per-basket Open Graph cards without prerendering. Every shared link (any basket, any page) shows the single site-level OG card from `index.html`. `sitemap.xml` is regenerated by every build from the site URL (stub until one is set); `robots.txt` has a placeholder `Sitemap:` line to fill in manually. Per-basket OG cards are your own prerender/edge choice (see `03-IMPLEMENTATION.md`).
