...
// THE LAST GATE lives in signable-confirm.ts now (the split's boundary work —
// both products run the same final displayed-vs-signed question, so it moved
// to the seam; its full calling contract, including the captured-when-rendered
// rule and the f(x) === f(x) trap, travels with it). Re-exported unchanged.
export { confirmSignableAmount } from './signable-confirm'
