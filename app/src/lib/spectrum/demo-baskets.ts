...
//
// Twenty HAND-COMPOSED baskets, not random seeds: each one is a deliberate
// look, so every tab in the trailer is visibly a different site. Variety is
// engineered along the axes that actually show on camera:
...
// The twenty. Ordered so consecutive tabs look maximally different — colour
// family and leg count both change every row, which is what a cut between tabs
// actually shows.
const DEMO: DemoSpec[] = [
  {
 ...
 *  films the same chart. 15 hourly points, one optional swing. */
function series(seed: number, drift: number, chop: number): NavPoint[] {
  const rng = mulberry32(seed * 2654435761)
  const nowSec = Math.floor(Date.now() / 1000)
  const swingAt = rng() < 0.5 ? 4 + Math.floor(rng() * 8) : -1
  const swing = (rng() - 0.45) * chop * 3
  let v = 100
  const out: NavPoint[] = []
  for (let i = 0; i < 15; i++) {
    v += (rng() - 0.5) * chop + drift / 14
    if (i === swingAt) v += swing
    out.push({ time: nowSec - (14 - i) * 3600, value: Math.max(1, v) })
  }
  return out
}
...
export function demoBasketUrls(origin = 'http://localhost:5311'): { label: string; url: string }[] {
  return DEMO.map((spec, i) => ({
    label: `${spec.name} ($${spec.symbol}) · ${spec.legs.length} legs · chain ${spec.chainId}`,
