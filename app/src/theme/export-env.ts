...
export interface DeployConfig {
  feeWallet: string
  /** Alchemy key (VITE_ALCHEMY_API_KEY). The convenience path — covers Base + Ethereum. */
  rpcKey: string
  /** Full per-chain endpoint URLs from ANY provider (QuickNode, Infura, a node you run…).
   *  Beat the key when set (VITE_BASE_RPC_URL / VITE_MAINNET_RPC_URL / VITE_ROBINHOOD_RPC_URL);
   *  either rail satisfies the RPC requirement and unlocks full V4 coverage. */
  rpcUrlBase: string
  rpcUrlMainnet: string
  rpcUrlRobinhood: string
  siteUrl: string
  tier: FeeTier
}
...
export const DEFAULT_DEPLOY: DeployConfig = {
  feeWallet: '', rpcKey: '', rpcUrlBase: '', rpcUrlMainnet: '', rpcUrlRobinhood: '', siteUrl: '', tier: 'all',
}
...
export function validateDeploy(d: DeployConfig): { errors: DeployIssues; warnings: DeployIssues } {
  ...
  // The RPC requirement is satisfied by EITHER rail: an Alchemy key, or at least one
  // custom provider URL (QuickNode / Infura / your own node — the per-chain fields).
  const anyRpcUrl = !!(d.rpcUrlBase.trim() || d.rpcUrlMainnet.trim() || d.rpcUrlRobinhood.trim())
  if (!d.rpcKey.trim() && !anyRpcUrl)
    errors.rpcKey = 'Required. An Alchemy key here, or your own provider URLs below (either ships in the public bundle; restrict it to your domain)'
  else if (/^https?:\/\//i.test(d.rpcKey.trim()))
    errors.rpcKey = 'That looks like a URL. This field takes a bare Alchemy key; full provider URLs go in the custom RPC fields below'
  for (const [field, v] of [
    ['rpcUrlBase', d.rpcUrlBase],
    ['rpcUrlMainnet', d.rpcUrlMainnet],
    ['rpcUrlRobinhood', d.rpcUrlRobinhood],
  ] as const) {
    if (v.trim() && !isHttpsUrl(v.trim())) warnings[field] = 'Should be a full https:// endpoint URL'
  }
...
export function envConfigToText(d: DeployConfig): string {
  ...
  L.push('# RPC (required, either rail) — ships in the public bundle; restrict it to your domain.')
  L.push('# Rail 1: an Alchemy key. Rail 2: full per-chain endpoint URLs from ANY provider')
  L.push('# (QuickNode, Infura, a node you run) — the URL beats the key when both are set.')
  L.push('# These stay out of git (this file is ignored): on a git-connected CI build, set')
  L.push('# the same vars in the host dashboard/CLI.')
  L.push(`VITE_ALCHEMY_API_KEY=${d.rpcKey.trim()}`)
  L.push(`VITE_BASE_RPC_URL=${d.rpcUrlBase.trim()}`)
  L.push(`VITE_MAINNET_RPC_URL=${d.rpcUrlMainnet.trim()}`)
  L.push(`VITE_ROBINHOOD_RPC_URL=${d.rpcUrlRobinhood.trim()}`, '')
