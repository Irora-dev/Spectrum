...
function startCartesianLoop({
  ...
  const paintFill = (intensity: number, reveal: number) => {
    ...
    s.configKeys.forEach((key, si) => {
      ...
      // constituent-gradient fill (owner 2026-07-29): a per-column colour ramp
      // across the basket's asset colours, weight-proportioned
      const ramp = s.seedRampOf(key, cols)
      ...
      for (let x = 0; x < cols; x++) {
        ...
        paintColumn(octx, x, Math.min(a, b), Math.max(a, b), ramp ? ramp[x] : seed, {
          variant,
          intensity,
          dim,
  ...
  const draw = (now: number) => {
    ...
    const s = state.current
    if (!s.ready) return
    // PERF (owner 2026-07-29, a grid of sparks must not lag machines): once
    // the entrance has played and nothing is hovered, idle ambience (winking
    // stars, settled fills) repaints at ~24fps instead of every frame.
    if (entranceReported && !s.isMouseInChart && !s.hovered && now - lastIdleFrame < 42) return
    lastIdleFrame = now
    if (resumePending) {
      resumePending = false
      // paused mid-entrance: wall-clock progress would snap to done — replay
      // the sweep from the resume instant (audit #8)
      if (!entranceReported) {
        animStart = 0
        lastProg = -1
      }
    ...
    if (mx >= 0 && mx <= revealCols) {
      for (const key of s.configKeys) {
        const cur = current[key]
        if (!cur) continue
        const rampM = s.seedRampOf(key, cols)
        const seed = rampM ? rampM[mx] : s.seedOf(key)
  ...
  raf = requestAnimationFrame(draw)
  // PERF: charts outside the viewport stop their loop completely — a page of
  // sparks only pays for the ones on screen. Rehydrates with a small margin so
  // scrolling into view never shows a stalled frame.
  let onScreen = true
  ...
  const io =
    typeof IntersectionObserver !== "undefined"
      ? new IntersectionObserver(
          (entries) => {
            // act on the LATEST record — a fast flick can batch [leave, enter]
            // and entries[0] would freeze a visible chart (audit #7)
