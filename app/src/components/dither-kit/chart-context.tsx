...
import { seedOfColor, seedRamp } from "./palette"
import {
  buildBandScale,
  buildXScale,
  buildYScale,
...
export type ChartConfig = Record<string, { label?: string; color: SeriesColor; palette?: { color: string; weight: number }[] }>
...
export type ChartContextValue = {
  ...
  max: number
  min: number // the data minimum (in "zero" mode the y-scale clamps it to ≤0)
  ...
  /** Per-column Seed ramp for a series with a constituent palette (else null). */
  seedRampOf: (key: string, cols: number) => Seed[] | null
...
export function useChartController({
  ...
  const { bands, max, min } = useMemo(
    () => computeBands(data, configKeys, stackType, yDomain),
    [data, configKeys, stackType, yDomain]
  )
  ...
  // Ramps are read on EVERY hovered frame (the marker path) — cache per
  // key+cols; the cache's identity follows config, so invalidation is exact
  // (audit 2026-07-29: ~1600 allocations per uncached call, 60+/s hovered).
  const rampCache = useMemo(() => new Map<string, Seed[] | null>(), [config])
  const seedRampOf = useCallback(
    (key: string, cols: number) => {
      const ck = `${key}:${cols}`
      const hit = rampCache.get(ck)
      if (hit !== undefined) return hit
      const pal = config[key]?.palette
      let ramp: Seed[] | null = null
      if (pal && pal.length >= 2) {
        const r = seedRamp(pal, cols)
        ramp = r.length > 0 ? r : null
      }
      rampCache.set(ck, ramp)
      return ramp
    },
    [config, rampCache],
  )
