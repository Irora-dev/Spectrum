...
export function ThesisEditor({
  ...
  if (!open) {
    ...
    // card's top right (the host section is position:relative).
    return (
      <button
        type="button"
        onClick={() => setOpen(true)}
        aria-label={meta?.thesis ? 'Edit your thesis' : 'Write your thesis'}
        className="press absolute right-0 top-2.5 inline-flex items-center gap-1.5 rounded-lg border border-cyan/30 bg-cyan/[0.06] px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan hover:border-cyan/60"
      >
  ...
  return (
    <div className="mt-3 space-y-2">
      <textarea
        ...
        className="min-h-[100px] w-full resize-y rounded-xl border border-white/12 bg-black/40 px-3.5 py-2.5 text-sm leading-relaxed text-ink placeholder:text-ink-faint focus:border-cyan/60 focus:outline-none"
      />
